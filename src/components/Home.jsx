...
import { BarChart2, Clock, CheckCircle, ChevronDown, Zap, Menu } from "lucide-react";
...
const NavItem = ({ title, items, onItemClick }) => {
  ...
  const handleClick = (item) => {
    if (item === "Job Sheet") navigate("/jobsheet");
    if (onItemClick) onItemClick(item);
  };
  return (
    <div className="relative px-4 py-2 cursor-pointer" onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
    ...
    <div className="min-h-screen flex flex-col bg-slate-950 text-white">
      ...
      <nav className="sticky top-0 z-50 h-16 flex items-center justify-between px-4 backdrop-blur-xl bg-slate-900/70 border-b border-white/5">
        ...
        <div className="hidden md:flex items-center">
          ...
          />
          <div onClick={() => navigate("/report")} className="px-4 py-2 cursor-pointer text-slate-300 hover:text-indigo-400 text-sm">Report</div>
          <div onClick={handleLogout} className="px-4 py-2 cursor-pointer text-red-400 hover:text-red-500 text-sm">Logout</div>
        </div>
      ...
      {mobileMenu && (
        <div className="md:hidden bg-slate-900 border-b border-white/10 flex flex-col">
          <div onClick={() => { navigate("/jobsheet"); setMobileMenu(false); }} className="px-4 py-3 text-slate-300 border-b border-white/5">Job Sheet</div>
          {role === "admin" && (
            <>
          ...
          <div onClick={() => { setShowMakeModal(true);   setMobileMenu(false); }} className="px-6 py-2 text-slate-400">Make</div>
          <div onClick={() => { setShowModelModal(true);  setMobileMenu(false); }} className="px-6 py-2 text-slate-400">Model</div>
          <div onClick={() => { setShowDrawerModal(true); setMobileMenu(false); }} className="px-6 py-2 text-slate-400 border-b border-white/5">Drawer</div>
          <div onClick={() => navigate("/report")} className="px-4 py-3 text-slate-300">Report</div>
          <div onClick={() => { handleLogout(); setMobileMenu(false); }} className="px-4 py-3 text-red-400">Logout</div>
        </div>
      )}
